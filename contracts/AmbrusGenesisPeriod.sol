...
abstract contract AmbrusGenesisInterface {
    function mint(address address_, uint256 agpTokenId_)
        public
        virtual
        returns (uint256);
}

contract AmbrusGenesisPeriod is ERC721A, Ownable, Pausable, ReentrancyGuard {
    ...
    struct RevealConfig {
        uint64 startTime;
        address ambrusGenesisContractAddress;
    }
    ...
    constructor(string memory baseURI_, WhitelistSaleConfig memory whiteSaleConfig_) ERC721A("Ambrus Genesis Period", "AGP") {
        baseURI = baseURI_;
    ...
    function reveal(uint256 tokenId_) external callerIsUser nonReentrant returns (uint256) {
        ...
        _burn(tokenId_);
        AmbrusGenesisInterface ambrusGenesisContract = AmbrusGenesisInterface(revealConfig.ambrusGenesisContractAddress);
        uint256 ambrusGenesisContractTokenId = ambrusGenesisContract.mint(_msgSender(), tokenId_);
        return ambrusGenesisContractTokenId;
    }
    ...
    function isRevealEnabled() public view returns (bool) {
        return revealConfig.startTime > 0 &&
            ...
            revealConfig.ambrusGenesisContractAddress != address(0);
    }
